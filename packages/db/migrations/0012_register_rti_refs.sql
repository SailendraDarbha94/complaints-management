-- 0012_register_rti_refs.sql
--
-- Fill the column the register has been holding open.
--
-- 0005 shipped the register view with `NULL::text AS "RTI refs."` and a comment saying
-- Phase 5, because the shape of the book had to be stable from the first export even
-- though there was no table behind that column. 0011 built the table. This is the column
-- being connected to it, and nothing else changes: same view, same columns, same order,
-- same types, so every export and every screen reading it is unaffected.
--
-- The view body below is 0005's, copied mechanically with two edits - the column
-- expression, and the lateral join that feeds it. It is restated in full because
-- CREATE OR REPLACE VIEW has no way to change one column of an existing view.
--
-- Why this matters more than a blank cell: the register export is what an RTI reply is
-- assembled from. A book that cannot say which RTI applications have already asked about
-- a case is a book that answers the same application twice, differently.
