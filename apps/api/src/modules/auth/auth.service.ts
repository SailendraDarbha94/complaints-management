...
export class AuthService {
  ...
  async verifyCode(
    ...
    if (!otp || otp.expired) throw new UnauthorizedException(BAD_CODE);
  ...
  async refresh(
    ...
    if (!session) throw new UnauthorizedException('Session not found. Sign in again.');
    ...
    if (memberships.length === 0) {
      await db.execute(sql`
        ...
        WHERE family_id = ${session.family_id}::uuid AND revoked_at IS NULL
      `);
      throw new UnauthorizedException('Your council membership has ended.');
    }
  ...
  async switchCouncil(
    ...
    const me = user.rows[0];
    if (!me) throw new UnauthorizedException('Sign in again.');

    const memberships = await this.membershipsFor(me.email.toLowerCase());
    const target = memberships.find((m) => m.councilId === councilId);
    if (!target) throw new UnauthorizedException('You are not a member of that council.');
